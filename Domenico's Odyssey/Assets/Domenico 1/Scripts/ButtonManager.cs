...
public class ButtonManager : MonoBehaviour {
	...
	private List<ButtonData> currentButtonsInUi = new List<ButtonData>();
	public ColorDB db;
	private List<int> indexColors = new List<int>();
	[FormerlySerializedAs("timerSwap")] [SerializeField]
	Vector2 timerEvent = new Vector2(3, 5);
	...
	[Header("Animation")] public float yMovement;
	public float yTime;
	public float xTime;
	...
	private void Start()
	{
		...
		for (int i = 0; i < buttons.Count; i++) {
			...
			currentButtonsInUi.Add(buttons[i]);
		}
		currentOutButton = buttons[^1];
		currentButtonsInUi.Remove(currentOutButton);
		currentOutButton.isOutOFRange = true;
		currentTimerSwap = Random.Range(timerEvent.x, timerEvent.y);
		characterController.OnPlayerWin += ChangeStatusButton;
	}
	...
	public IEnumerator AnimThrowOut(ButtonData button, ButtonData button2) {
		...
		if (button.isIn) {
			...
			currentButtonsInUi.Add(button2);
			currentButtonsInUi.Remove(button);
		}
		else {
			inButton = button2;
			outButton = button;
			...
			currentButtonsInUi.Add(button);
			currentButtonsInUi.Remove(button2);
		}
